...
from the nucleus of the paired galaxies. Code is specifically to generate the 
figures for different galaxy mass ratios.
...
fig, axs = plt.subplots(2, 2, sharex=False, sharey=False, figsize=(16, 16))
for j in range(len(bining)):
    ...
    elif bining[j] == 'dm_abs':
        label = '$|\Delta$log(M)| = '
        binsize = 0.25
        mbins = np.arange(0, 1.0, binsize) + binsize/2
        z = np.abs(np.concatenate((dms[0], dms[1]))) 
        
    dexbin, dexstd = f.stack(data, z=z, zbin=mbins, error='sem')
    dexbinf, dexstdf = f.stack(data, z=z, zbin=mbins, error='sem', trim=0.5)
    
    # Dex plot
    axs[j,0].plot(rbins, profile, c='k', zorder=9999, linestyle='--', linewidth=2.0)
    for i in range(len(mbins)):
        n = len(data[np.abs(mbins[i]-z) <= binsize/2])
        if len(dexbin[i])>1:
            axs[j,0].plot(rbins, dexbin[i], c=colors[i], label=label+str(mbins[i]-binsize/2)+'$-$'+str(mbins[i]+binsize/2)+' ('+str(n)+')')
            #axs[j,0].plot(rbins, dexbin[i], c=colors[i], linestyle='--')
            axs[j,0].fill_between(rbins, dexbin[i]-dexstd[i], dexbin[i]+dexstd[i], alpha=0.2,
                             antialiased=True)
            
    leg = axs[j,0].legend(handlelength=0, handletextpad=0, fontsize=fontsize/2.0, loc=3, fancybox=False, edgecolor='k')
    ...
    axs[j,0].plot([-0.1,3.0], [0,0], c='k', linestyle=':') 
    axs[j,0].axvline(1.5, linestyle='-.', c='k')
    
    axs[j,0].set_xlabel(r'$\rm R/R_{eff}$', fontsize=fontsize)
    axs[j,0].set_xlim(-0.1,2.7)
    
    axs[j,0].set_ylabel(r'$\rm \Delta log(sSFR, dex)$', fontsize=fontsize)
    rng = [-11.15, -9.35]
    #axs[0].set_ylim(-1*(rng[1]-rng[0])/2, (rng[1]-rng[0])/2)
    axs[j,0].set_ylim(-0.65, 0.65)
    loc1 = 0.5
    loc2 = 0.1
#______________________________________________________________________________
    """ Nuc Data"""
    yrng = [-1.45, 1.45]
    axs[j,1].set_ylim(yrng)
    ...
    if bining[j] == 'dm':
        ...
        axs[j,1].set_xlim(-1.6, 2.2)
        axs[j,1].plot([-2.6, 2.6], [med,med], 'k--', linewidth=2.0, zorder=9999)
        axs[j,1].plot([-2.6, 2.6], [0,0], 'k:', linewidth=2.0, zorder=9999)
        axs[j,1].plot([0,0], yrng, 'k:', linewidth=2.0, zorder=9999)
        ps.ticks(axs[j,1], xmajor=1.0, ymajor=0.5, xminor=0.2, yminor=0.1)
        axs[j,1].set_xlabel(r'$\Delta$log(M)', fontsize=fontsize)
        
    elif bining[j] == 'dm_abs':
        binsize = 0.25
        mbins = np.arange(0, 1.75, binsize) + binsize/2
        label = '$|\Delta$log(M)| = '
        z = np.abs(dm)
        
        axs[j,1].set_xlim(-0.1, 2.1)
        axs[j,1].plot([-2.6, 2.6], [med,med], 'k--', linewidth=2.0, zorder=9999)
        axs[j,1].plot([-2.6, 2.6], [0,0], 'k:', linewidth=2.0, zorder=9999)
        axs[j,1].plot([0,0], yrng, 'k:', linewidth=2.0, zorder=9999)
        ps.ticks(axs[j,1], xmajor=0.5, ymajor=0.5, xminor=0.1, yminor=0.1)
        axs[j,1].set_xlabel(r'$|\Delta$log(M)|', fontsize=fontsize)
#______________________________________________________________________________    
    
    axs[j,1].errorbar(z[0:dms[0].shape[0]], nuc_data[0:dms[0].shape[0]], yerr=std[0:dms[0].shape[0]], fmt='s', c='deepskyblue', label='Outside-IFU Pairs')
    axs[j,1].set_ylabel(r'$\rm \Delta log(sSFR, dex)$', fontsize=fontsize)
    ...
    for i in range(len(mbins)):
        ...
        if len(y) > 0:
            ...
            y = np.nanmedian(y[np.isfinite(y)])
            x = mbins[i]
            axs[j,1].errorbar(x, y, xerr=binsize/2.0, yerr=ystd, c='k', ms=10, marker='D', zorder=9999, capsize=5.0, capthick=2.0, elinewidth=2.0)
        
    leg = axs[j,1].legend(handlelength=0, fontsize=fontsize/2.0, loc=3, fancybox=False, edgecolor='k', borderpad=0.5)
    for line, text in zip(leg.get_lines(), leg.get_texts()):
        text.set_color(line.get_color())
    for item in leg.legendHandles:
        item.set_visible(False)
...
plt.savefig(outpath + '/ssfr_dm.pdf', bbox_inches='tight', overwrite=True)
plt.close('all')